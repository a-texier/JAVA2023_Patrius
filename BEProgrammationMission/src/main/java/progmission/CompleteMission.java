...
public class CompleteMission extends SimpleMission {
	...
	private EventDetector createConstraintXDetector(Site targetSite) {
		...
		/*SensorModel sensorModel = new SensorModel(satellite.getAssembly(), "sensor");

		CelestialBody terre = null;
		try {
			terre = CelestialBodyFactory.getEarth();
		...
		sensorModel.addMaskingCelestialBody(terre.getShape());

        // target
        GeodeticPoint targ = targetSite.getPoint();

        // Repère topocentrique basé sur la target
        final TopocentricFrame targFrame = new TopocentricFrame(terre.getShape(), targ, targ.getName());
        final CircularField targField = new CircularField("TargetField",
                FastMath.PI / 3, Vector3D.PLUS_K);
	...
	private EventDetector createConstraintSunIncidenceDetector(Site targetSite) throws PatriusException {
